...
docker run -d --name filemin -p 10000:10000 dejan995/filemin-docker:latest >/dev/null
sleep 5

rm -rf 'xmlrpc.cgi'
echo "Downloading xmlrpc.cgi using default authentication"
wget --no-check-certificate https://admin:admin@localhost:10000/xmlrpc.cgi >/dev/null
sleep 5
ls -lh 'xmlrpc.cgi'
file 'xmlrpc.cgi'
rm 'xmlrpc.cgi'
...
echo "Running filemin in the background"
docker run -d --name filemin -p 10000:10000 -e WEBMIN_LOGIN=dinosaurs -e WEBMIN_PASSWORD=are-awesome dejan995/filemin-docker:latest >/dev/null
sleep 5

rm -rf 'xmlrpc.cgi'
echo "Downloading xmlrpc.cgi using custom authentication"
wget --no-check-certificate https://dinosaurs:are-awesome@localhost:10000/xmlrpc.cgi >/dev/null
sleep 5
ls -lh 'xmlrpc.cgi'
file 'xmlrpc.cgi'
rm 'xmlrpc.cgi'
